        ...
        {isDesktop ? (
                ...
                {images.map((element, index) => {
                  return (
                    <div className={styles.productBadgeslist} key={`image-${index}`}>
                      <img src={element.src} alt={element.alt} className={styles.imgMax} width={'110px'} height={'110px'}/>
                    </div>
                  )
                })}
              </div>
          ...
          <Carousel arrows={false} customButtonGroup={<ButtonGroup />} itemClass="m-0" autoPlay={true} infinite={true} containerClass="container"  autoPlaySpeed={3000}
            ...
            {images.map((element,i1) => {
              return (
                  <img src={element.src} alt={element.alt} className={styles.imgMax}  key={i1} width={'110px'} height={'110px'}/>
              )
            })}
          </Carousel>
          </div>
