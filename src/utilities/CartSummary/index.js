...
const CartSummary = ({ data }) => {
    ...
    const Checkout = ()=>{
        ...
        myHeaders.append("X-Shopify-Customer-Access-Token", Decrypt());
    ...
    return (
        <div className={styles.cartSummaryContainer}>
            ...
            {/* {data.tax &&(
                <div className={styles.priceContainer}>
                <p>Tax:</p>
                <strong>{data.verdion =='EU' ?'€':'$'}{data?.tax || '0.00'}</strong>
            </div>
            )}
             {data?.total &&(
                <div className={styles.priceContainer} style={{borderTop:'1px solid #eee', paddingTop:'0.5rem'}}>
                <p style={{fontWeight:600}}>Total:</p>
                <strong>{data.verdion =='EU' ?'€':'$'}{data?.total || '0.00'}</strong>
            </div>
            )} */}

            <div className={styles.checkoutButtonContainer}>
                <Link href={data?.url || '/collections/all-products'}>
