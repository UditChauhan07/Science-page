...
export const CollectionList = ({ content }) => {
    ...
    return (
        <div ref={elementRef} className={styles.collectionContainer} style={content.length == 1 ?{  gridTemplateColumns: 'auto'}:{}}>
        {content.map((e,i)=>{
            if(!e?.hide){
            return(
                <a href={e.link ? e.link + e?.SLUG : '#'} key={i}>
                <div className={styles.collectionCard} style={content.length == 1 ?{  width: '280px'}:{}}>
