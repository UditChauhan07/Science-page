...
const Collection = () => {
    ...
    useEffect(() => {
        if (!collezioni) {
            ...
            if (splitUrl.length == 2) {
                ...
                if (splitUrl1[0]) {
                    setCategory({
                        name: collection[splitUrl1[0]]?.title ? splitUrl1[0] : false,
                        DataisLoaded: true
                    })
                } else {
                    setCategory({
    ...
    const { img, wistia,video } = masterHead || {};
        ...
        <section>
            {name ?
                <>
                    ...
                    {(wistia?.id || video) && <WistiaHero id={wistia.id} content={wistia?.content} video={video}/>}
